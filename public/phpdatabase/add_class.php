...
$location_date_idx =  $conn->insert_id;
...
if ($stmt_class = $conn->prepare("INSERT INTO class (location_date_id, name, departament, course, semester) VALUES (?, ?, ?, ?, ?)")) {
	$stmt_class->bind_param("isssi", $location_date_idx, $_POST['name'], $_POST['departament'], $_POST['course'], $_POST['semester']);
	$stmt_class->execute();
} else {
...
if ($stmt_lecturer_classes = $conn->prepare("INSERT INTO lecturer_classes (lecturer_id, class_id) VALUES (?, ?)")) {
	$stmt_lecturer_classes->bind_param("ii", $lecturer_idx, $class_idx);
